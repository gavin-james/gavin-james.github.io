<template><div><nav class="table-of-contents"><ul><li><router-link to="#职责链模式基本介绍">职责链模式基本介绍</router-link></li><li><router-link to="#职责链模式的结构与实现">职责链模式的结构与实现</router-link></li><li><router-link to="#学校-oa-系统的采购审批项目">学校 OA 系统的采购审批项目</router-link></li><li><router-link to="#职责链模式解决采购审批项目">职责链模式解决采购审批项目</router-link></li><li><router-link to="#职责链模式的注意事项和细节">职责链模式的注意事项和细节</router-link></li><li><router-link to="#职责链模式的应用场景">职责链模式的应用场景</router-link></li></ul></nav>
<h2 id="职责链模式基本介绍" tabindex="-1"><a class="header-anchor" href="#职责链模式基本介绍" aria-hidden="true">#</a> 职责链模式基本介绍</h2>
...
<p>在计算机软硬件中也有相关例子，如总线网中数据报传送，每台计算机根据目标地址是否同自己的地址相同来决定是否接收；还有异常处理中，处理程序根据异常的类型决定自己是否处理该异常；还有 Struts2 的拦截器、JSP 和 Servlet 的 Filter 等，所有这些都可以考虑使用职责链模式来实现。</p>
<p><strong>职责链模式</strong>（Chain of Responsibility Pattern）又叫 <strong>责任链模式</strong>，为了避免请求发送者与多个请求处理者耦合在一起，于是将所有请求的处理者通过前一对象记住其下一个对象的引用而连成一条链；当有请求发生时，可将请求沿着这条链传递，直到有对象处理它为止。这种模式对请求的发送者和接收者进行解耦。</p>
<p>在职责链模式中，通常每个接收者都包含对另一个接收者的引用。客户只需要将请求发送到职责链上即可，无须关心请求的处理细节和请求的传递过程，请求会自动进行传递，如果一个对象不能处理该请求，那么它会把相同的请求传给下一个接收者，依此类推。所以职责链将请求的发送者和请求的处理者解耦了。</p>
...
<li>职责链简化了对象之间的连接。每个对象只需保持一个指向其后继者的引用，不需保持其他所有处理者的引用，这避免了使用众多的 if 或者 if-else 语句</li>
...
<h2 id="职责链模式的结构与实现" tabindex="-1"><a class="header-anchor" href="#职责链模式的结构与实现" aria-hidden="true">#</a> 职责链模式的结构与实现</h2>
<p>职责链模式主要包含以下角色：</p>
<ul>
...
<p>其结构图如下图所示：</p>
<p><img src="https://fastly.jsdelivr.net/gh/Kele-Bingtang/static/img/design-pattern/20220326184617.png" alt="image-20220326184616770" loading="lazy"></p>
<p>客户端可按下图所示设置职责链：</p>
<p><img src="https://fastly.jsdelivr.net/gh/Kele-Bingtang/static/img/design-pattern/20220326184655.png" alt="image-20220326184654465" loading="lazy"></p>
<blockquote>
<p>代码实现</p>
</blockquote>
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token comment">// 抽象处理者角色</span>
...
<span class="token punctuation">}</span>
<span class="token comment">// 具体处理者角色 1</span>
<span class="token keyword">class</span> <span class="token class-name">ConcreteHandler1</span> <span class="token keyword">extends</span> <span class="token class-name">Handler</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
<span class="token comment">// 具体处理者角色 2</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ChainOfResponsibilityPattern</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="学校-oa-系统的采购审批项目" tabindex="-1"><a class="header-anchor" href="#学校-oa-系统的采购审批项目" aria-hidden="true">#</a> 学校 OA 系统的采购审批项目</h2>
<blockquote>
<p>采购员采购教学器材</p>
</blockquote>
<ul>
<li>如果金额小于等于 5000，由教学主任审批（<code v-pre>0 &lt;= x &lt;= 5000</code>）</li>
<li>如果金额小于等于 10000，由院长审批（<code v-pre>5000 &lt; x &lt;= 10000</code>）</li>
<li>如果金额小于等于 30000，由副校长审批（<code v-pre>10000 &lt; x &lt;= 30000</code>）</li>
<li>如果金额超过 30000 以上，有校长审批（<code v-pre>30000 &lt; x</code>）</li>
</ul>
<blockquote>
<p>传统方案题分析</p>
</blockquote>
...
<li>传统方式的问题分析：客户端这里会使用到分支判断（比如 Switch）来对不同的采购请求处理，这样就存在如下问题：
<ul>
...
<p>解决方案：<strong>职责链模式</strong>。</p>
<h2 id="职责链模式解决采购审批项目" tabindex="-1"><a class="header-anchor" href="#职责链模式解决采购审批项目" aria-hidden="true">#</a> 职责链模式解决采购审批项目</h2>
<p>思路分析和类图：</p>
<p><img src="https://fastly.jsdelivr.net/gh/Kele-Bingtang/static/img/design-pattern/20220326183930.png" alt="image-20220326183929550" loading="lazy"></p>
<p>代码实现：</p>
<p>职责链类</p>
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">abstract</span> <span class="token keyword">class</span> <span class="token class-name">Approver</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
<span class="token comment">// 学校管理者（权力最大）</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SchoolMasterApprover</span> <span class="token keyword">extends</span> <span class="token class-name">Approver</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
<span class="token comment">// 学校副管理者（权力第二）</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ViceSchoolMasterApprover</span> <span class="token keyword">extends</span> <span class="token class-name">Approver</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
<span class="token comment">// 院系管理者（权力第三）</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CollegeApprover</span> <span class="token keyword">extends</span> <span class="token class-name">Approver</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
<span class="token comment">// 班级管理者（权力第四）</span>
...
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token comment">// 请求类</span>
...
<p>SpringMVC 的 HandlerExecutionChain 类就使用到职责链模式。</p>
</blockquote>
<h2 id="职责链模式的注意事项和细节" tabindex="-1"><a class="header-anchor" href="#职责链模式的注意事项和细节" aria-hidden="true">#</a> 职责链模式的注意事项和细节</h2>
<ul>
<li>职责链模式将请求和处理分开，实现解耦，提高系统的灵活性</li>
<li>职责链模式简化了对象，使对象不需要知道链的结构</li>
<li>性能会受到影响，特别是在链比较长的时候，因此需控制链中最大节点数量，一般通过在 Handler 中设置一个最大节点数量，在 <code v-pre>setNext()</code> 方法中判断是否已经超过阀值，超过则不允许该链建立，避免出现超长链无意识地破坏系统性能</li>
<li>调试不方便，采用了类似递归的方式，调试时逻辑可能比较复杂</li>
</ul>
<h2 id="职责链模式的应用场景" tabindex="-1"><a class="header-anchor" href="#职责链模式的应用场景" aria-hidden="true">#</a> 职责链模式的应用场景</h2>
