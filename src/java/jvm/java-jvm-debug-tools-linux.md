...
```bash
[root@root ~]# netstat -nat|awk  '{print $6}'|sort|uniq -c|sort -rn
      5 LISTEN
      2 ESTABLISHED
      1 Foreign
      1 established)
...
```bash
[root@root ~]# grep MemTotal /proc/meminfo
MemTotal:        1882088 kB
```
...
```bash
[root@root ~]# grep MemFree /proc/meminfo
MemFree:           74120 kB
```
